...
def schedule(tasks, collectors, preferences):
    ...
    for shift_index, shift_end in enumerate(unique_shift_ends):
        ...
        tasks_excl_partially_processed = tasks_excl_partially_processed.sort_values(
            by=['processing_deadline', 'percentage_complete'],
            ascending=[True, False]
        )
        ...
        for index in range(len(tasks_excl_fully_processed)):
            capacity_used += 1
            task = tasks_excl_fully_processed.iloc[index]
